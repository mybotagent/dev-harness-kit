...
**Current human-use skills** (22): `bootstrap`, `ci-setup`, `plan`, `build`, `review`, `security`, `audit`, `inspect`, `eval`, `report`, `repair`, `ship`, `status`, `onboard`, `config`, `shortcut-quick-fix`, `shortcut-tdd-fast`, `feat-add`, `feat-fix`, `feat-revise`, `feat-remove`, `adapt`.
...
- `tests/test_smoke.py` enforces: exactly `SKILL_COUNT` skills total (37 as of #<feat/report-html-renderer PR>; bump both test_smoke.py and this rule together when adding a new skill).
