...
**Switching the CI review provider** (MiniMax / Anthropic / DeepSeek): run `bin/set-provider.sh <provider>` — it edits the git-tracked `.github/ci-review-provider.txt`, prints the diff for review, then you commit + push yourself. **`.env` is not consulted** for provider selection (a GitHub-hosted runner can't see your local `.env` anyway). Each provider needs its matching repo secret (`MINIMAX_API_KEY`, `ANTHROPIC_API_KEY`, or `DEEPSEEK_API_KEY`) pushed via `gh secret set` first. Note: a PR that itself edits `.github/workflows/review.yml` will always get skipped by `claude-code-action`'s own anti-tampering guard ("workflow file must match the default branch") — that's expected and resolves itself once such a PR is merged.
