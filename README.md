...
### Symptom → escape hatch (already-merged consumer is red)

If the consumer repo's CI is **already red** with a missing-file error like `scripts/branch-policy.sh: No such file or directory`, the root cause is almost always a stale plugin cache: a previous `ci-setup` ran with an old dev-kit SHA that didn't include the file. Recovery:

```bash
# 1. Refresh the plugin cache (root cause)
bin/devkit-refresh.sh
# In-session: /reload-plugins

# 2. Re-run ci-setup --force to install ALL 15 files
cd /path/to/consumer-repo
claude --plugin-dir /Users/sanghee/dev/dev-harness-kit
/dev-kit:ci-setup --force

# 3. Verify the missing file is now present + syntactically valid
ls scripts/branch-policy.sh && bash -n scripts/branch-policy.sh && echo "✅ OK"

# 4. If --force did NOT install it (very rare; means cache still stale),
#    manually copy from dev-kit templates as an escape hatch:
mkdir -p scripts
cp /Users/sanghee/dev/dev-harness-kit/templates/ci/scripts/branch-policy.sh scripts/
chmod +x scripts/branch-policy.sh

# 5. Commit + push
git add scripts/branch-policy.sh
git commit -m "fix(ci): add missing branch-policy.sh"
git push
```

Steps 1–3 fix root cause. Step 4 is the escape hatch when the cache cannot be refreshed in time. Step 5 is mandatory regardless of which path restored the file.
