...
- **docs(docs-maintenance)**: Refresh stale documentation against current implementation (PR #TBD). Stale counts and paths fixed: README hook intro "the 8 hooks below" → "the hooks below" (table now lists 9 of 14 active entries; count is volatile so the prose no longer pins a specific number); LCS section "5 core production handlers" → "6 (`worktrees`, `branches`, `pr`, `sessions`, `spend`, `valuations`)"; `marketplace.json` path → `.claude-plugin/marketplace.json`; "Three dimensions" agent-behavior eval section now clarifies that `evaluate` adds the `harness-quality` and `os-quality` dimensions on the same `--dim` flag. `skills/lcs/SKILL.md` URI table now lists `lcs://valuations/<plan-id>`, and three "five production" mentions updated to "six"; trailing placeholder cruft (lines 88-99 from before PR #436) removed. `bin/dev-kit-lcs.py` and `tests/test_dev_kit_lcs_cli.py` docstring counts aligned with the actual 6-resource registry. `docs/skills/README.md` dropped the stale `maintenance` row (the workflow is documented at `docs/quality/maintenance-gate.md`, not as a skill). `skills/README.md` regenerated from `SKILL.md` frontmatter via `tools/build_skills_readme.py` (line-based parser tolerates unquoted colons in descriptions): now shows 40 skills across 13 categories (35 human-invocable, 5 model-invoked), auto-sorted by category + alphabetical. New file: `tools/build_skills_readme.py` (134 lines). Verified: `python3 bin/dev-kit-lcs.py --list-resources` returns 6 handlers; `pytest tests/test_smoke.py tests/test_naming.py tests/test_skill_governance.py tests/test_dev_kit_lcs_cli.py` → 30 passed, 1 skipped; `pytest tests/test_lcs_*.py` → 207 passed in 54.6 s. Pre-existing drifts flagged for follow-up (not changed in this PR): `LICENSE` file referenced by the MIT badge does not exist; `skills/lcs/SKILL.md` is missing the `user-invocable: false` and `model:` frontmatter fields that the README implies; `.github/workflows/maintenance.yml` + `docs/quality/maintenance-gate.md` + `docs/stages/STAGES.md` + `eval/prompts/judge-maintenance.md` all reference `/dev-kit:maintenance` for which no `skills/maintenance/` exists.
- **feat(lcs)**: Aggregation responses now carry a `summary` block (active/stale counts, slot_drift, as_of) so operators read freshness at a glance instead of eyeballing per-row timestamps (PR #459, closes #455 Gap 2). New `lib/lcs_resources/_summary.py` helper parses slot versions as dotted tuples (`0.3.150 > 0.3.100` numerically, not lexicographically) and computes `active` from a 24h cutoff against `as_of`. `lcs://worktrees` collection form gains `{worktrees, summary}`; the `<branch>` item form is unchanged. 17 tests in `tests/test_lcs_summary.py` cover the 14-row proposal example, empty-list edge case, None/unparseable slot versions, and the 24h window.

- **feat(lcs)**: List variants for `<id>`-keyed resources (PR #460, closes #455 Gap 3). `lcs://branches`, `lcs://sessions`, and `lcs://prs` each return a small summary-shaped list payload (not the full per-record detail), letting the operator discover ids without grepping `logs/` or shelling out to `gh pr list`. Empty index returns `summary.total: 0` (honest data, not an error); `LCSPartialError` semantics stay reserved for partial info on a real record. Drill in with the `<id>`-keyed variant for the full payload. 24 tests in `tests/test_lcs_discovery.py` plus operator reference at `docs/lcs/lcs-list-endpoints.md`.

- **feat(lcs)**: `--list-routes` flag and reserved-URI honesty in `bin/dev-kit-lcs.py` (PR #458, closes #455 Gap 4). A documented URI that is not a registered route is worse than no URI -- operators read the SKILL.md, type the URI, get exit 2, and conclude LCS is broken. The new flag prints `registered:` vs `reserved (not implemented):` sections; `skills/lcs/SKILL.md` now marks `lcs://hooks/coverage`, `lcs://interview/<step>`, and `lcs://research/cache` explicitly as reserved rather than advertising them as callable. 7 tests in `tests/test_lcs_routes_flag.py` cover both sections and the doc-vs-flag alignment.

- **feat(lcs)**: NL-to-URI router with break-even rule (PR #457, closes #455 Gap 1). New `bin/dev-kit-lcs-route.py` is a thin deterministic classifier (no LLM, no ML -- keyword/regex patterns + URI lookup table) that fires only when a question maps to a multi-source aggregation no single shell call produces. **Not a skill** -- a skill that decides whether to call another skill is L6 anti-pattern; the router is a CLI binary that respects the break-even rule "if a single shell call answers the question, use the shell; if it requires N correlated calls across heterogeneous sources, route to LCS". Costs ~200 tokens per routed call; ROI is positive only when it replaces N correlated shell calls. 17 tests in `tests/test_lcs_route.py` pin the proposal's break-even table plus fallback + empty-input + `--list-rules` behavior.

- **docs(lcs)**: LCS UX proposal at `docs/proposals/lcs-ux/router-and-summaries.{yaml,html}` (branch `feat/lcs-ux-proposal`, issue #455). Documents the four gaps (router, summaries, discovery, reserved-URI honesty) and the recommended merge order.
...
- **docs(public)**: `README.md`, `docs/quality/ci-setup.md`, `docs/hooks/hook-coverage-gaps.md` updated.

### Changed — absorb `feat-add` / `feat-fix` / `feat-revise` into `prune` (feat/feat-absorb, issue #284)

The `/dev-kit:feat-add`, `/dev-kit:feat-fix`, and `/dev-kit:feat-revise` slashes were three thin TDD wrappers that shared an underlying contract with `feat-remove`: read plan artifacts, find the named boundary, route the work. With `prune` now adding a single-feature deletion mode via its `--target <feat>` flag, the same single-entrypoint story applies to fix / revise / add — and the L6 alpha gate (`analysis` is the easy skill to write and the easiest for next-gen models to absorb) discourages new forks. All three skills retire; their roles move to `prune` (deletion) and `/dev-kit:build` / `/dev-kit:plan` (TDD greenfield / revision).

- **chore(skills)**: `skills/feat-add/SKILL.md`, `skills/feat-fix/SKILL.md`, `skills/feat-revise/SKILL.md` deleted.
- **feat(skills)**: `skills/prune/SKILL.md` — 3-phase → 4-phase. New phases `[2/4] DEPENDENTS` (call-graph walk via `skills/prune/scripts/discover_dependents.py`, gates Phase 3 on per-row user ack) and `[4/4] VERIFY` (run the full test suite; route failures to `/dev-kit:build-debug`). The `--target <feat>` mode adds an inlined single-feature path alongside the existing `/dev-kit:feat-remove` slash: same call-graph sweep + deletion report, but inlined into the prune flow with mandatory Phase 4 full-suite gate.
- **feat(skills)**: `skills/prune/scripts/discover_dependents.py` — Phase 2 helper. Thin wrapper over `lib/analysis_core.runner.run_analysis(mode="delete", ...)`. Reads a Phase-1 candidate JSON (object or flat-list shape), invokes the engine's `inspect` dimension set in delete mode, renders a Markdown DEPENDENTS block with one row per finding plus the engine verdict. Exits 2 on usage errors (bad `--candidates` path) so the SKILL.md body can route Phase 2 to the right hand-off.
- **test**: `tests/test_prune.py` — locks in the 4-phase contract (`[1/4]`, `[2/4]`, `[3/4]`, `[4/4]` headings, phase-name assertions, `test_target_flag_absorbs_feat_remove` for the migration, `test_phase4_runs_full_suite` for the full-suite gate, plus a positive-path coverage of the new `discover_dependents.py` script via `test_prune_target_runs_full_suite` and a missing-candidates exit-2 test).
...
- **docs**: `skills/build/SKILL.md`, `skills/ci-setup/SKILL.md`, `skills/audit-outdated/SKILL.md`, `README.md`, `docs/quality/ci-setup.md` updated — pre-flight gates now read "marker absent" only, no version comparison language.
...
- **docs(adr)**: ADR-0010 + docs/naming/NAMING.md example lists updated (`build-simplify` → `build-refactor`; `simplify` → `refactor`; add `prune` + `build-prune`).
