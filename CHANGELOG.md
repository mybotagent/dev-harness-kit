...
- **fix(ci)**: paginate verdict extraction + babysit verdict freshness check (#572). SHO-178: `.github/workflows/review.yml` (lines 215, 366) and `.github/workflows/cost-flag.yml` (line 181) now pass `per_page=100` so the comment-page-1 extraction does not read a stale claude[bot] verdict. SHO-179: `lib/babysit_pr_reliability.py` gains `check_verdict_freshness()` (FRESH / STALE / GHOST) consumed by `skills/babysit-pr/SKILL.md` step 8.5; details in the SKILL.md spec.
...
## babysit iteration 1785902024 — force judge rerun
