...
- **fix(ci)**: paginate verdict extraction + babysit verdict freshness check (#572). SHO-178: `.github/workflows/review.yml` had two `gh api .../issues/<PR>/comments` calls (security verdict at line 215 and review verdict at line 366) and `.github/workflows/cost-flag.yml` had one (existing-comment lookup at line 181) that did not pass `per_page=100`, so any PR with >30 comments extracted a stale page-1 claude[bot] verdict instead of the latest one. All three are now `?per_page=100`. SHO-179: `lib/babysit_pr_reliability.py` gains a `check_verdict_freshness(pr_number, run_id, run_started_epoch, now_epoch)` helper plus `FRESH`/`STALE`/`GHOST` status strings and `VERDICT_FRESHNESS_WINDOW_SECONDS=600`. `skills/babysit-pr/SKILL.md` step 8.5 mandates the babysit re-fetch `gh pr checks` and run the freshness check before claiming 'all green' — before this, the babysit would echo `judge_verdict=Requested` from a stale page-1 comment while the actual Claude comment on page 2 said `Approve`. Six tests in `tests/test_babysit_pr_reliability.py::TestCheckVerdictFreshness` + `TestIsStaleLock` pin the contract.
