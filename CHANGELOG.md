...
The `/dev-kit:feat-add`, `/dev-kit:feat-fix`, and `/dev-kit:feat-revise` slashes were three thin TDD wrappers that shared an underlying contract with `feat-remove`: read plan artifacts, find the named boundary, route the work. With `prune` now absorbing single-feature deletion via its `--target <feat>` flag, the same single-entrypoint story applies to fix / revise / add — and the L6 alpha gate (`analysis` is the easy skill to write and the easiest for next-gen models to absorb) discourages new forks. All three skills retire; their roles move to `prune` (deletion) and `/dev-kit:build` / `/dev-kit:plan` (TDD greenfield / revision).
