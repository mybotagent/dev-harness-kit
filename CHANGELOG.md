...
- **feat(skills)**: `skills/prune/SKILL.md` — 3-phase → 4-phase. New phases `[2/4] DEPENDENTS` (call-graph walk via `skills/prune/scripts/discover_dependents.py`, gates Phase 3 on per-row user ack) and `[4/4] VERIFY` (run the full test suite; route failures to `/dev-kit:build-debug`). The `--target <feat>` flag replaces the old `/dev-kit:feat-remove` slash: same call-graph sweep + deletion report, but inlined into the prune flow with mandatory Phase 4 full-suite gate.
