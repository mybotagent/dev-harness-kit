...
Issues are correlated by an exact `<!-- scope:<branch>::auto-sync -->` description marker. Queries are scoped to `LINEAR_PROJECT_NAME` (default `dev-harness-kit`) and paginated. Workflow-state IDs are resolved only within the project's team.
