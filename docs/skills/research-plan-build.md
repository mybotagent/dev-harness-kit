...
1. **Phase 1 — Research.** Reads `lib/research_engine.py` + `lib/analysis_core/runner.py`, then writes `.dev-kit/hand-off/<session>/research.md` from `templates/research.md`. The research engine runs the Phase 0-3 escalation (cache / direct / multi / human) and `enforce_citations()`. Every claim in the Conclusion section must cite `url` + `fetched_at` + `source_type`, OR be flagged `[UNCITED]`. The gate to advance is `enforce_citations(text)` returning zero uncited sentences in the Conclusion. The phase refuses to write any source file under `src/`, `lib/`, `tests/`, `hooks/`, `skills/<other>/`, `commands/`, `tools/`.
...
- **Phase 3**: per-step branches + 2-commit protocol per row of the Steps table (delegated to `/dev-kit:build`); each commit body references `plan.md step N` and the matching acceptance criteria.

Failure exit codes surface as errors from any phase that fails its gate (uncited conclusion, missing step rows, missing `verification:` fields, or a build-runner contract violation).
...
- `skills/build/SKILL.md` references `research-plan-build` under the composition section.
