...
def _render_iron_laws(iron_laws: List[str]) -> str:
    """§1 body — Iron Laws list. Header + numbered items + footer."""
...
def _render_codebase_map_index(project_root: Path) -> str:
    """§3 body — lazy-loading index. Delegates to render_stub_section_3."""
    return (
        "## §3 Codebase Map (lazy-loading index)\n"
        "\n"
        f"{render_stub_section_3(project_root)}"
    )


def _render_hook_matrix(hook_matrix: Optional[str] = None) -> str:
    """§4 body — Hook Matrix (active-hooks.json SSOT)."""
    body = hook_matrix if hook_matrix is not None else render_hook_matrix_table()
    return f"## §4 Hook Matrix (active-hooks.json SSOT, MUST-13)\n\n{body}\n"
...
def _render_shared_rules(project_root: Path) -> str:
    """Shared-rules block (AGENTS.md pointer + per-rule read bullets)."""
    if (project_root / "rules").is_dir():
        shared_rules = sorted(
...
def render_claude_md(
    ...
    """Compose full CLAUDE.md content. Thin 5-line dispatcher (issue #97).

    `full_map` is accepted for back-compat but no longer affects §3 content —
    ...
    sections = "\n".join((
        _render_iron_laws(laws),
        _render_active_stage(stage),
        _render_codebase_map_index(project_root),
        _render_hook_matrix(hook_matrix),
        _render_handoff(hand_off_chain),
        _render_shared_rules(project_root),
    ))
    return f"{_HEADER}\n{sections}{_FOOTER}"
