...
name: adapt
category: build
description: Mid-build plan/spec amendment. Pauses the current step, diffs PRD.md + step file against actual output, proposes a minimal patch, writes it on user approval, and resumes build-engine.
when_to_use: |
...
[4/4] APPLY    → on explicit user approval:
                    ...
                    - resume /dev-kit:build (build-engine flips blocked → pending → in_progress)
```
...
- The skill does NOT modify `step<N>-output.json`. The build-engine owns that file.
...
- Updated `phases/<name>/index.json` flipping the step from `blocked` back to `pending` (build-engine picks it up).
