...
when_to_use:
  ...
  - After /dev-kit:ci-setup or /dev-kit:bootstrap (with ci-setup prompt) to verify readiness
...
**0-arg. Read-only. Never mutates target files; never writes secrets; never opens PRs.** Issue #212-D1: a consumer who just ran `/dev-kit:bootstrap (with ci-setup prompt)` has no way to ask "is my CI ready?" until the next PR turns red. This skill answers that question in one call.
...
- **`/dev-kit:bootstrap (with ci-setup prompt)` should run this skill next**: it is the canonical post-install verification (issue #212-D1 / D2).
