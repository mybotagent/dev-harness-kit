...
3. **Auto-Done** — when the prompt contains a completion verb (`done`, `finished`, `complete[d]?`, `shipped`, `merged`, `closed`) AND no work verb is also present (work-verb-wins prevents accidental Done when the prompt is really starting new work), the matching issue is transitioned to `Done` and the sync round exits without further mutations.
