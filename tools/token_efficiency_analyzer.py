...
# `.worktree/` is the new client-neutral canonical (issue #272). Keep the
...
WORKTREE_ROOT_NAMES = (".worktree", ".worktrees", ".claude/worktrees", ".codex/worktrees")
...
def _worktree_marker(parts: tuple[str, ...]) -> tuple[str, int] | None:
    for i, part in enumerate(parts):
        if part in {".worktree", ".worktrees"} and i + 1 < len(parts):
            return parts[i + 1], i
...
def repo_from_cwd(cwd: str | None) -> str:
    ...
    For a session running inside a worktree (``.worktree/<name>/`` or a legacy root)
...
def worktree_from_cwd(cwd: str | None) -> str:
    ...
    A worktree in this repo lives under ``<repo>/.worktree/<name>/`` (legacy: ``.worktrees/``, ``.claude/worktrees/``, ``.codex/worktrees/``)
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--include-worktree-logs", action=argparse.BooleanOptionalAction,
                        default=True,
                        help="Auto-discover logs from .worktree/*/logs/ and legacy worktree roots (.worktrees/, .claude/worktrees/, .codex/worktrees/) (default: True). "
