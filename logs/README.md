...
|---|---|---|
| `claude-code/` | Claude Code `Stop` + `SessionEnd` hooks | One `.jsonl` per session |
| `codex/` | Codex `Stop` + `SessionEnd` hooks | One `.jsonl` per session |
...
# ... use Claude Code / Codex — transcripts land in logs/<tool>/<sid>.jsonl ...
